...
/*i2c read*/
unsigned int write_read(hi_u8 reg_high_8bit_cmd, hi_u8 reg_low_8bit_cmd, hi_u8* recv_data, hi_u8 send_len, hi_u8 read_len)
{
    hi_i2c_idx id = 0;
...
unsigned int c08i_nfc_i2c_write( hi_u8 reg_high_8bit_cmd, hi_u8 reg_low_8bit_cmd, hi_u8* data_buff, hi_u8 len)
{
    unsigned int status =0;
    hi_i2c_idx id = 0;
    hi_i2c_data c081nfc_i2c_write_cmd_addr ={0};
...
/* 写寄存器*/
unsigned int write_fifo_reg( hi_u8 reg_high_8bit_cmd, hi_u8 reg_low_8bit_cmd, hi_u8 data_buff)
{
    unsigned int status =0;
    hi_i2c_idx id = 0;
    hi_i2c_data c081nfc_i2c_write_cmd_addr ={0};
...
/*写fifo data*/
unsigned int write_fifo_data( hi_u8* data_buff, hi_u8 len)
{
    unsigned int status =0;
    hi_i2c_idx id = 0;
    hi_i2c_data c081nfc_i2c_write_cmd_addr ={0};
...
/*EEPROM page write*/
void eep_write_page(hi_u8 *pBuffer, unsigned short WriteAddr, hi_u8 datalen)
{
    unsigned int status;
...
/*写EEPROM*/
void fm11_write_eep(unsigned short addr,unsigned int len,hi_u8 *wbuf)
{
	hi_u8 offset;
...
/*读EEPROM*/
unsigned int fm11_read_eep(hi_u8 *dataBuff, unsigned short ReadAddr, unsigned short len)
{	
    unsigned int status;
...
/*读NFC寄存器*/
hi_u8 fm11_read_reg(unsigned short addr)
{
	hi_u8 pdata[10] ={0};
	hi_u8 a =0;
...
/*写NFC寄存器*/
hi_u8 fm11_write_reg(unsigned short addr, hi_u8 data)
{
    unsigned int status =0;
...
/*数据回发*/
void fm11_data_send(unsigned int ilen,hi_u8 *ibuf)
{
	unsigned int slen =0;
	hi_u8 *sbuf = NULL;
...
/*读取RF数据*/
unsigned int fm11_data_recv(hi_u8 *rbuf)
{
	hi_u8 irq =0;
    hi_u8 ret =0;
	hi_u8 irq_data_wl = 0;
...
// static void NFCTask(void *arg)
{
    ...
    IoTGpioInit(14);
    hi_io_set_func(14, 6);
    IoTI2cInit(0, HI_I2C_IDX_BAUDRATE);
...
static void NFCDemo(void)
{
    ...
    if (osThreadNew(NFCTask, NULL, &attr) == NULL) {
        printf("[NFCTask] Falied to create NFCTask!\n");
    }
    osKernelUnlock();
#else
    unsigned int ret =0;
    ...
    hi_task_lock();
    attr.stack_size = 10*1024;
    attr.task_prio = osPriorityNormal;
    attr.task_name = (hi_char*)"app_nfc_display";
    ret = hi_task_create(&g_nfc_display_task_id, &attr, NFCTask, HI_NULL);
    if (ret != HI_ERR_SUCCESS) {
